...
class StudentsBaseInfoRule(object):
    ...
    async def get_students_base_info_by_student_id(self, student_id) -> StudentsBaseInfo:
        ...
        schoolcominfo = await self.school_commu_dao.get_school_communication_by_school_id(
            students_base_info_db.school_id)
        if schoolcominfo:
            students_base_info.loc_area = schoolcominfo.loc_area
            students_base_info.loc_area_pro = schoolcominfo.loc_area_pro
        convert_snowid_in_model(students_base_info,
                                ["id", 'student_id', 'school_id', 'class_id', 'session_id', 'student_base_id',
                                 'grade_id'])
    ...
    async def add_students_base_info(self, students_base_info: NewBaseInfoCreate):
        ...
        students_base_info_db = view_model_to_orm_model(students_base_info, StudentBaseInfo,
                                                        exclude=["student_base_id"])
        # 读取当前开启的届别  赋值
