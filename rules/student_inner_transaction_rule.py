...
class StudentInnerTransactionRule(object):
    ...
    async def get_student_inner_transaction_by_id(self, student_inner_transaction_id):
        student_inner_transaction_db = await self.student_inner_transaction_dao.get_student_inner_transaction_by_id(
    ...
    async def get_student_inner_transaction_by_student_inner_transaction_name(self, student_inner_transaction_name):
        student_inner_transaction_db = await self.student_inner_transaction_dao.get_student_inner_transaction_by_student_inner_transaction_name(
            student_inner_transaction_name)
        student_inner_transaction = orm_model_to_view_model(student_inner_transaction_db, StudentInnerTransactionModel,
                                                            exclude=[""])
        return student_inner_transaction
    ...
    async def add_student_inner_transaction(self, student_inner_transaction: StudentInnerTransactionModel):
        ...
        student_inner_transaction_db = view_model_to_orm_model(student_inner_transaction, StudentInnerTransaction,
                                                               exclude=["id"])
    ...
    async def query_student_inner_transaction_with_page(self, page_request: PageRequest,
                                                        student_inner_transaction_search):
        paging = await self.student_inner_transaction_dao.query_student_inner_transaction_with_page(
            student_inner_transaction_search, page_request)
    ...
    async def query_student_inner_transaction(self, student_inner_transaction_name):
        ...
        result = await session.execute(select(StudentInnerTransaction).where(
            StudentInnerTransaction.student_inner_transaction_name.like(f'%{student_inner_transaction_name}%')))
