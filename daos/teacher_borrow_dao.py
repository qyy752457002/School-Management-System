...
class TeacherBorrowDAO(DAOBase):
	...
	async def get_teacher_borrow_by_teacher_borrow_id(self, teacher_borrow_id):
		session = await self.slave_db()
		result = await session.execute(select(TeacherBorrow).where(TeacherBorrow.teacher_borrow_id == teacher_borrow_id))
	...
	async def update_teacher_borrow(self, teacher_borrow, *args, is_commit=True):
		...
		query = update(TeacherBorrow).where(TeacherBorrow.teacher_borrow_id == teacher_borrow.teacher_borrow_id).values(**update_contents)
