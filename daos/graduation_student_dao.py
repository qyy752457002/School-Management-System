...
class GraduationStudentDAO(DAOBase):
    ...
    async def get_graduationstudent_count(
        self,
    ):
        session = await self.slave_db()
        result = await session.execute(
            select(func.count()).select_from(GraduationStudent)
        )
    ...
    async def get_graduationstudent_by_id(self, id):
        ...
        result = await session.execute(
            select(GraduationStudent).where(
                GraduationStudent.id == id, GraduationStudent.is_deleted == False
            )
        )
    ...
    async def get_graduationstudent_by_student_id(self, student_id):
        ...
        result = await session.execute(
            select(GraduationStudent).where(
                GraduationStudent.student_id == student_id,
    ...
    async def get_graduationstudent_by_name(self, name):
        session = await self.slave_db()
        result = await session.execute(
            select(GraduationStudent).where(GraduationStudent.student_name == name)
        )
        return result.scalar_one_or_none()

    async def query_graduationstudent_with_page(
        self, page_request: PageRequest, **kwargs
    ):
        query = (
            select(
                Student.student_id,
                ...
                StudentBaseInfo.class_id,
                StudentBaseInfo.school_id,
                StudentBaseInfo.graduation_type,
                School.school_name,
                School.block,
                School.borough,
                Classes.class_name,
            ...
            .join(School, School.id == StudentBaseInfo.school_id, isouter=True)
            .join(
                PlanningSchool,
                School.planning_school_id == PlanningSchool.id,
                isouter=True,
            )
            .join(Classes, Classes.id == StudentBaseInfo.class_id, isouter=True)
        )

        query = query.where(
            Student.approval_status == StudentApprovalAtatus.GRADUATED.value
        )
    ...
    async def update_graduationstudent(self, graduationstudent, *args, is_commit=True):
        ...
        query = (
            ...
            .where(StudentBaseInfo.student_id == graduationstudent.student_id)
            .values(**update_contents)
        )
        return await self.update(
            session, query, graduationstudent, update_contents, is_commit=is_commit
        )
    ...
    async def softdelete_graduationstudent(self, graduationstudent):
        ...
        update_stmt = (
            ...
            .where(GraduationStudent.id == graduationstudent.id)
            .values(
    ...
    async def update_graduation_student_archive_status(self, borough, is_commit=True):
        ...
        try:
            stmt_select = select(GraduationStudent).where(
                GraduationStudent.borough == borough,
                GraduationStudent.archive_status == False,
            )
            result = await session.execute(stmt_select)
            students_to_update = result.scalars().all()
    ...
    async def update_graduation_student_by_school_id(
        ...
        try:
            ...
            stmt_select = (
                ...
                .join(
                    StudentSession,
                    StudentSession.session_id == StudentBaseInfo.session_id,
                )
    ...
    async def query_graduation_student_by_model_with_page(
        ...
        query = (
            select(
                GraduationStudent.student_id,
                GraduationStudent.school_id,
                GraduationStudent.school,
                GraduationStudent.student_name,
                GraduationStudent.status,
                GraduationStudent.archive_status,
                GraduationStudent.graduation_date,
                Classes.class_name,
            )
            .join(Classes, Classes.id == GraduationStudent.class_id)
            .where(GraduationStudent.is_deleted == False)
        )
        ...
        if query_model.student_name:
            query = query.where(
                GraduationStudent.student_name == query_model.student_name
            )
        if query_model.graduation_date_s and query_model.graduation_date_e:
            query = query.where(
                GraduationStudent.graduation_date.between(
                    query_model.graduation_date_s, query_model.graduation_date_e
                )
            )
        ...
        if query_model.archive_status is not None:
            query = query.where(
                GraduationStudent.archive_status == query_model.archive_status
            )
    ...
    async def query_school_archive_status_with_page(
        self, page_request: PageRequest, query_model: CountySchoolArchiveQueryModel
    ):
        relationship_count_subquery = (
            select(
                CountyGraduationStudent.school_id,
                CountyGraduationStudent.graduate_count.label("graduate_count"),
                func.count(CountyGraduationStudent.school_id).label("relation_count"),
            )
            .where(CountyGraduationStudent.year == str(datetime.now().year))
            .group_by(CountyGraduationStudent.school_id)
            .subquery()
        )
        archive_status = case(
            (relationship_count_subquery.c.relation_count > 0, True), else_=False
        ...
        query = (
            select(
                ...
                School.id,
                archive_status,
                func.coalesce(relationship_count_subquery.c.graduate_count, 0).label(
                    "graduate_count"
                ),
            )
            .join(
            ...
            .where(
                ...
                School.institution_category == None,
    ...
    async def get_school_is_graduate(self, query_archive_status=False):
        ...
        relationship_count_subquery = (
            select(
                GraduationStudent.school_id,
                func.count(GraduationStudent.school_id).label("relation_count"),
            )
            .where(GraduationStudent.graduation_year == str(datetime.now().year))
            .group_by(GraduationStudent.school_id)
            .subquery()
        )
        archive_status = case(
            (relationship_count_subquery.c.relation_count > 0, True), else_=False
        ...
        query = (
            select(School.school_name, School.id, archive_status)
            .join(
                relationship_count_subquery,
                School.id == relationship_count_subquery.c.school_id,
                isouter=True,
    ...
    async def upgrade_all_student(self, school_id: int):
        ...
        for class_obj in classes_to_upgrade:
            session_info = await session_dao.get_student_session_by_id(
                class_obj.session_id
            )
            section = session_info.section
            grade_level = Section.get_grade_level(section)
            current_grade = await grade_dao.get_grade_by_id_and_school_id(
                class_obj.grade_id, school_id, session_info.section
            )
            if current_grade:
                if grade_level != 0:
                    # 如果学制不是0，代表不是中职学校
                    if current_grade.grade_index < grade_level:
                        # 如果当前年级小于学制年级，升级
                        update_grade = await grade_dao.get_grade_by_index_and_school_id(
                            current_grade.id + 1, school_id, section
                        )
                        if update_grade:
                            # todo 更新班级的年级
                            update_contents = {"grade_id": update_grade.id}
                            await session.update(Classes).where(
                                Classes.id == class_obj.id
                            ).values(**update_contents)
                            # todo 更新同一班级学生的年级
                            await session.update(StudentBaseInfo).where(
                                StudentBaseInfo.class_id == class_obj.id
                            ).values(**update_contents)
                        else:
                            continue
