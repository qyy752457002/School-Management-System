...
class CourseDAO(DAOBase):
    ...
    async def get_course_by_id(self, id):
        session = await self.slave_db()
        result = await session.execute(select(Course).where(Course.id == int(id)))
        return result.scalar_one_or_none()

    async def get_course_by_school_id(self, id):
        session = await self.slave_db()
        result = await session.execute(select(Course).where(Course.school_id == int(id), Course.is_deleted == False))
        return result.scalar_one_or_none()
