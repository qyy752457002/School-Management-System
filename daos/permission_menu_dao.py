...
class PermissionMenuDAO(DAOBase):
	...
	async def query_permission_menu_with_args(self,  unit_type, edu_type, system_type, role_id: int = None,parent_id=0):
		query = (select(PermissionMenu.id,
                        ...
                        Role.app_name
                        ).select_from( PermissionMenu).join(RolePermission, RolePermission.menu_id == PermissionMenu.id, isouter=True).join(Role, Role.id == RolePermission.role_id, isouter=True).order_by(asc(RolePermission.sort_order)).order_by(asc(PermissionMenu.sort_order)).order_by(asc(RolePermission.id)))
