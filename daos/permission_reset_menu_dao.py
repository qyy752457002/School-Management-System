...
class PermissionResetMenuDAO(DAOBase):
    ...
    async def query_permission_menu_with_page(self, page_request: PageRequest, unit_type, edu_type, system_type,
                                              role_id: int = None, ):
        query = select(PermissionResetMenu).join(RolePermissionReset,
                                                 RolePermissionReset.menu_id == PermissionResetMenu.id,
    ...
    async def query_permission_menu_with_args(self, unit_type, edu_type, system_type, role_id: int = None, parent_id=0,
        ...
        query = (select(PermissionResetMenu.id,
                        ...
                        Role.app_name
                        ).select_from(PermissionResetMenu).join(RolePermissionReset,
                                                                RolePermissionReset.menu_id == PermissionResetMenu.id,
                                                                isouter=True).join(Role,
                                                                                   Role.id == RolePermissionReset.role_id,
                                                                                   isouter=True).order_by(
            asc(RolePermissionReset.sort_order)).order_by(asc(PermissionResetMenu.sort_order)).order_by(
            asc(RolePermissionReset.id)))
        ...
        if system_type:
            if system_type == "student" and unit_type == "" and edu_type == "":
                query = query.where(Role.system_type == system_type, Role.edu_type.not_in(["vocational"]))
            else:
                query = query.where(Role.system_type == system_type)
        if unit_type:
            query = query.where(Role.unit_type == unit_type)
    ...
    async def update_permission_menu(self, permission_menu, *args, is_commit=True):
        ...
        query = update(PermissionResetMenu).where(PermissionResetMenu.id == permission_menu.id).values(
            **update_contents)
