...
class OrganizationMembersDAO(DAOBase):
    ...
    async def get_organization_members_count(self, org_id):
        ...
        result = await session.execute(select(func.count()).select_from(OrganizationMembers).where(
            OrganizationMembers.org_id == int(org_id)).where(OrganizationMembers.is_deleted == False))
    ...
    async def update_organization_members(self, organization_members, *args, is_commit=True):
        ...
        query = update(OrganizationMembers).where(OrganizationMembers.id == organization_members.id).values(
            **update_contents)
