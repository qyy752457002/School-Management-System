...
class PlanningSchoolEduinfoDAO(DAOBase):
    ...
    async def get_planning_school_eduinfo_by_id(self, planning_school_eduinfo_id):
        planning_school_eduinfo_id = int(planning_school_eduinfo_id)

        session = await self.slave_db()
        result = await session.execute(
            select(PlanningSchoolEduinfo).where(PlanningSchoolEduinfo.id == planning_school_eduinfo_id))
    ...
    async def get_planning_school_eduinfo_by_planning_school_id(self, planning_school_eduinfo_id):
        ...
        session = await self.slave_db()
        result = await session.execute(
            select(PlanningSchoolEduinfo).where(PlanningSchoolEduinfo.planning_school_id == planning_school_eduinfo_id))
        return result.scalar_one_or_none()
    ...
    async def update_planning_school_eduinfo(self, planning_school_eduinfo, ctype=1):
        ...
        # session.add(planning_school_eduinfo)
        if ctype == 1:
            update_stmt = update(PlanningSchoolEduinfo).where(
                PlanningSchoolEduinfo.id == planning_school_eduinfo.id).values(
    ...
    async def query_planning_school_eduinfo_with_page(self, planning_school_eduinfo_name, planning_school_eduinfo_id,
                                                      planning_school_eduinfo_no,
    ...
    async def update_planning_school_eduinfo_byargs(self, planning_school_eduinfo: PlanningSchoolEduinfo, *args,
                                                    is_commit: bool = True):
        session = await self.master_db()
        ...
        if planning_school_eduinfo.planning_school_id > 0:
            query = update(PlanningSchoolEduinfo).where(
                PlanningSchoolEduinfo.planning_school_id == planning_school_eduinfo.planning_school_id).values(
                **update_contents)
        else:
            query = update(PlanningSchoolEduinfo).where(PlanningSchoolEduinfo.id == planning_school_eduinfo.id).values(
                **update_contents)
        return await self.update(session, query, planning_school_eduinfo, update_contents, is_commit=is_commit)
