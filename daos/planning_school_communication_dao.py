...
class PlanningSchoolCommunicationDAO(DAOBase):
    ...
    async def get_planning_school_communication_by_id(self, planning_school_communication_id):
        ...
        result = await session.execute(select(PlanningSchoolCommunication).where(
            PlanningSchoolCommunication.id == planning_school_communication_id))
    ...
    async def get_planning_school_communication_by_planning_shool_id(self, planning_school_communication_id):
        ...
        session = await self.slave_db()
        result = await session.execute(select(PlanningSchoolCommunication).where(
            PlanningSchoolCommunication.planning_school_id == planning_school_communication_id))
        return result.scalar_one_or_none()
    ...
    async def update_planning_school_communication(self, planning_school_communication, ctype=1):
        ...
        # session.add(planning_school_communication)
        if ctype == 1:
            update_stmt = update(PlanningSchoolCommunication).where(
                PlanningSchoolCommunication.id == planning_school_communication.id).values(
    ...
    async def query_planning_school_communication_with_page(self, planning_school_communication_name,
                                                            planning_school_communication_id,
                                                            planning_school_communication_no,
                                                            page_request: PageRequest) -> Paging:
        query = select(PlanningSchoolCommunication)
        if planning_school_communication_name:
            query = query.where(
                PlanningSchoolCommunication.planning_school_communication_name == planning_school_communication_name)
        ...
        if planning_school_communication_no:
            query = query.where(
                PlanningSchoolCommunication.planning_school_communication_no == planning_school_communication_no)
        paging = await self.query_page(query, page_request)
        return paging

    async def update_planning_school_communication_byargs(self,
                                                          planning_school_communication: PlanningSchoolCommunication,
                                                          *args, is_commit: bool = True):
        session = await self.master_db()
        ...
        if planning_school_communication.planning_school_id > 0:
            ...
            query = update(PlanningSchoolCommunication).where(
                PlanningSchoolCommunication.planning_school_id == planning_school_communication.planning_school_id).values(
                **update_contents)

        else:

            query = update(PlanningSchoolCommunication).where(
                PlanningSchoolCommunication.id == planning_school_communication.id).values(**update_contents)
