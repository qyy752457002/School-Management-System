...
class StudentTransactionDAO(DAOBase):
    ...
    async def get_studenttransaction_by_process_instance_id(self, id):
        ...
        result = await session.execute(
            select(StudentTransaction).where(StudentTransaction.process_instance_id == int(id)).order_by(
                StudentTransaction.id.desc()))
        return result.scalar()

    async def get_studenttransaction_by_student_id(self, id):
        session = await self.slave_db()
        query =select(StudentTransaction).where(StudentTransaction.student_id == int(id)).order_by(
            StudentTransaction.id.desc()).where(StudentTransaction.is_deleted==False)
    ...
    async def query_studenttransaction_with_page(self, page_request: PageRequest, **kwargs, ):
        query = select(
            StudentTransaction.id, StudentTransaction.school_name, StudentTransaction.grade_name,
            StudentTransaction.classes, StudentTransaction.transfer_time, StudentTransaction.transfer_reason,
            StudentTransaction.doc_upload, StudentTransaction.student_id, StudentTransaction.student_no,
            # StudentTransaction.student_name,
            StudentTransaction.current_org, StudentTransaction.apply_user, StudentTransaction.apply_time,
            StudentTransaction.school_id, StudentTransaction.relation_id, StudentTransaction.transaction_type,
            StudentTransaction.transaction_type_lv2, StudentTransaction.country_no, StudentTransaction.reason,
            StudentTransaction.province_id, StudentTransaction.city_id, StudentTransaction.district_id,
            StudentTransaction.area_id, StudentTransaction.direction, StudentTransaction.transfer_in_type,
            StudentTransaction.session, StudentTransaction.attached_class, StudentTransaction.grade_id,
            StudentTransaction.class_id, StudentTransaction.major_id, StudentTransaction.major_name,
            StudentTransaction.remark, StudentTransaction.status, StudentTransaction.is_valid,
            StudentTransaction.created_uid, StudentTransaction.updated_uid, StudentTransaction.created_at,
            StudentTransaction.updated_at, StudentTransaction.is_deleted,
            ...
            StudentBaseInfo.edu_number,
                                               ...
                                               isouter=True).join(StudentBaseInfo,
                                                                  StudentBaseInfo.student_id == Student.student_id,
                                                                  isouter=True)
    ...
    async def update_studenttransaction(self, studenttransaction, *args, is_commit=True):
        ...
        query = update(StudentTransaction).where(StudentTransaction.id == int(studenttransaction.id)).values(
            **update_contents)
