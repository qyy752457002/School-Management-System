...
class TeachersInfoDao(DAOBase):
    ...
    async def query_teacher_with_page(self, query_model: NewTeacher, page_request: PageRequest) -> Paging:
        ...
        if query_model.id_number:
            query = query.where(Teacher.teacher_id_number == query_model.id_number)
        if query_model.gender:
            query = query.where(Teacher.teacher_gender == query_model.gender)
        if query_model.employer:
            query = query.where(Teacher.teacher_employer == query_model.employer)
        ...
        if query_model.approval_status:
            query = query.where(Teacher.teacher_approval_status == query_model.approval_status)
