...
class ClassDivisionRecordsImport(BaseModel):
    # 学生证件类型我这边 提供  班级id，学校no，学生证件号码，学生证件类型，我这边的学生id（可能以后关联用），学生来源？
    class_id: int |str= Field(0, title="班级ID", description="班级ID",examples=['1'])
    school_no: str = Field('', title="", description="学校no",examples=['1'])
    id_type: str = Field('', title="", description="学生证件类型",examples=['1'])
    id_number: str = Field('', title="", description="学生证件号码",examples=['1'])

    id:int|str= Field(0, title="", description="id",  )
    student_no: str = Field('', title="", description="学生编号",examples=['1'])
