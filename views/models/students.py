...
class NewStudentsQuery(BaseModel):
    ...
    # class_id: int | str |None = Query(0, title="", description="班级ID")
...
class StudentsFamilyInfo(BaseModel):
    ...
    relationship: Relationship | None = Field(..., title="关系", description="关系")
    ...
    birthday: date | None = Field(..., title="出生日期", description="出生日期")
...
"""
    ...
    admission_date: date | datetime | None | str | float = Field(date(1970, 1, 1), title="入学年月",
                                                                 description="入学年月new")
