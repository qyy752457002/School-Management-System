...
class PlanningSchool(BaseModel):
    ...
    created_uid: int |None= Field(..., title="创建人", description="创建人", examples=['1'])
    ...
    create_planning_school_date: str|None = Field(..., title="建校年月", description="建校年月",
                                             examples=['2021-10-10 00:00:00'])
    social_credit_code: str |None= Field(..., title="统一社会信用代码", description="统一社会信用代码",
                                    examples=['XH423423876867'])
    founder_type: str |None= Field(..., title="举办者类型", description="举办者类型", examples=['地方'])
    founder_type_lv2: str |None= Field(..., title="举办者类型二级", description="举办者类型二级", examples=['教育部门'])

    founder_type_lv3: str |None= Field(..., title="举办者类型三级", description="举办者类型三级", examples=['县级教育部门'])

    founder_name: str |None= Field(..., title="举办者名称", description="举办者名称", examples=['上海教育局'])
    founder_code: str |None= Field(..., title="举办者识别码", description="举办者识别码", examples=['SC562369322SG'])
    urban_rural_nature: str|None = Field(..., title="城乡性质", description="城乡性质", examples=['城镇'])
    planning_school_org_form: str|None = Field(..., title="办学组织形式", description="办学组织形式", examples=['教学点'])
    planning_school_closure_date: str|None = Field('', title="规划校关闭日期", description="规划校关闭日期", examples=[''])
    department_unit_number: str |None= Field(..., title="属地管理行政部门单位号", description="属地管理行政部门单位号",
                                        examples=['SC562369322SG'])
    sy_zones: str |None= Field(..., title="属地管理行政部门所在地地区", description="属地管理行政部门所在地地区",
                          examples=['铁西区'])
    historical_evolution: str|None = Field(..., title="历史沿革", description="历史沿革", examples=['xxxxxxxxxxxxxxxxxxxx'])
...
class PlanningSchoolBaseInfo(BaseModel):
    ...
    planning_school_edu_level: str | None = Query(..., title="", description="办学类型/规划校性质",
                                                  examples=['学前教育'])
...
class PlanningSchoolKeyInfo(BaseModel):
    ...
    planning_school_edu_level: str | None = Query(..., title="办学类型", description="办学类型/规划校性质",
                                                  examples=['学前教育']),
    planning_school_category: str | None = Query(..., title="办学类型二级", description=" 办学类型二级",
                                                 examples=['小学']),
