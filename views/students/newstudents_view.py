...
class NewsStudentsView(BaseView):
    ...
    async def page(self, student_name: str = Query(None, title="学生姓名", description="学生姓名", example="John Doe"),
                   ...
                   id_type: str = Query(None, title="证件类别", description="证件类别", example="ID Card"),
                   school: str = Query(None, title="学校", description="学校", example="清华"),
                   enrollment_date: date = Query(None, title="登记时间", description="登记时间",
                                                 example="2000-01-01 00:00:00"),
                   status: str = Query(None, title="状态", description="状态", example="流出"),
                   page_request=Depends(PageRequest)):
        print(page_request)
        items = []

        res = NewStudentsQuery(
    ...
    # 新生新增基本信息
    async def post(self, new_students_base_info: StudentsBaseInfo):
