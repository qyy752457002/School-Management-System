...
class StudentBaseInfo(BaseDBModel):
    ...
    registration_date: Mapped[date] = mapped_column(Date, nullable=False, comment="登记日期")
    ...
    enrollment_date: Mapped[date] = mapped_column(Date, nullable=False, comment="入学日期")
    ...
    overseas_chinese: Mapped[str] = mapped_column(String(64), nullable=False, comment="是否港澳台侨胞")
    residence_address_detail: Mapped[str] = mapped_column(String(64), nullable=True, comment="户口所在地（详细）")
    communication_district: Mapped[str] = mapped_column(String(64), nullable=False, comment="通信地址行政区")
    ...
    photo_upload_time: Mapped[str] = mapped_column(String(64), nullable=False, comment="照片上传时间")
